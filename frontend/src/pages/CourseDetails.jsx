...
function CourseDetails() {
  ...
  const handleCourseAction = async () => {
      ...
      if (isEnrolled) {
        //   const sectionId = courseData?.courseContent?.[0]?._id;
        //   const subSectionId = courseData?.courseContent?.[0]?.subSection?.[0]?._id;
    ...
    <div className="bg-richblack-900 min-h-screen">
                        ...
                        <div className="p-6 space-y-6">
                            <div className="text-3xl font-bold text-richblack-5">
                                {isEnrolled ? (
                                    ...
                                    Number(price) === 0 ? "Free" : `${Number(price).toLocaleString('en-US')} VND`
                                )}
                            </div>
                            <div className="flex flex-col gap-3">
                                ...
                                >
                                    {isEnrolled ? "Go to Course" : (Number(price) === 0 ? "Enroll Now" : "Buy Now")}
                                </button>
                                {!isEnrolled && Number(price) > 0 && (
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-richblack-800 p-4 border-t border-richblack-700 z-50 flex items-center justify-between gap-4 shadow-[0_-5px_20px_rgba(0,0,0,0.3)]">
         <div className="font-bold text-white text-lg">
            {isEnrolled ? "Purchased" : (Number(price) === 0 ? "Free" : `${Number(price).toLocaleString('en-US')} VND`)}
         </div>
         <button 
            onClick={handleCourseAction}
