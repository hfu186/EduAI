    ...
    if (formatVND(price) === formatVND(0)) {
      try {
        ...
        if (res?.success) {
          ...
          toast.error(
            res?.message || t("pages.course_details.errors.enroll_failed"),
          );
        }
      } catch (err) {
        ...
        <div className="mx-auto max-w-7xl px-4 py-4">
          <div className="flex items-center gap-2 text-richblack-300 text-sm mt-5">
            <Link to="/" className="hover:text-yellow-50 transition-colors">
              {t("pages.course_details.breadcrumb_home")}
            </Link>
            <BiChevronRight />
            <Link
              to="/all-courses"
            ...
            >
              {t("pages.course_details.breadcrumb_courses")}
            </Link>
            <BiChevronRight />
            ...
            <div className="lg:col-span-8 space-y-3">
              <div className="flex flex-wrap items-center gap-4 text-sm font-medium">
                <div className="flex items-center gap-2 bg-yellow-25/10 px-3 py-1.5 rounded-full border border-yellow-25/20 text-yellow-50">
                  ...
                  <span className="text-richblack-200 ml-1">
                    (
                    {t("pages.course_details.review_count", {
                      count: ratingAndReviews?.length || 0,
                    })}
                    )
                  </span>
                </div>
                <div className="flex items-center gap-2 bg-blue-100/10 px-3 py-1.5 rounded-full border border-blue-100/20 text-blue-100">
                  <FaUsers />
                  <span>
                    {t("pages.course_details.student_count", {
                      count: studentsEnrolled?.length || 0,
                    })}
                  </span>
                </div>
              ...
              <div className="flex flex-wrap items-center gap-6 pt-2 text-richblack-300 text-sm">
                <div className="flex items-center gap-2">
                  <span className="text-richblack-400">
                    {t("pages.course_details.created_by")}
                  </span>
                  <Link
                    to={`/instructor/${instructor?._id}`}
                    className="text-richblack-5 hover:underline cursor-pointer font-medium hover:text-caribbeangreen-200 transition-colors flex items-center gap-1"
                  >
                    {instructorName}
          ...
          <div className="lg:col-span-8 space-y-10">
            <div className="border border-richblack-700 bg-richblack-800/40 p-6 rounded-2xl shadow-xl backdrop-blur-md">
              <h2 className="text-2xl font-bold text-richblack-5 mb-3">
                {t("pages.course_details.what_youll_learn")}
              </h2>
              <div>
                {whatYouWillLearn &&
                  whatYouWillLearn.split("\n").map((line, index) => (
                    <div key={index} className="flex items-start gap-3">
                      ...
                      <p className="text-richblack-200 text-sm font-medium leading-6">
                        {line}
                      </p>
                    </div>
                  ))}
            ...
            <div className="space-y-4">
              <h2 className="text-2xl font-bold text-richblack-5">
                {t("pages.course_details.course_content")}
              </h2>
              <div className="flex flex-wrap justify-between items-center text-sm text-richblack-300 mb-4 bg-richblack-800/30 p-4 rounded-xl border border-richblack-700/50">
                <div className="flex gap-4 flex-wrap">
                  <span>
                    {t("pages.course_details.sections", {
                      count: courseContent?.length,
                    })}
                  </span>
                  <span>•</span>
                  <span>
                    {t("pages.course_details.lectures", {
                      count: totalNoOfLectures,
                    })}
                  </span>
                  <span>•</span>
                  <span>
                    {t("pages.course_details.total_length", {
                ...
                <button
                  onClick={() => setIsActive([])}
                  className="text-caribbeangreen-200 font-medium hover:text-caribbeangreen-300 transition-colors"
                >
              ...
              <div className="border border-richblack-700 rounded-xl overflow-hidden bg-richblack-900 shadow-md">
                {courseContent?.length > 0 ? (
                  ...
                  <div className="p-6 text-center text-richblack-400">
                    {t("pages.course_details.no_content")}
                  </div>
                )}
              </div>
            </div>
                ...
                <div className="flex-1">
                  ...
                  <p className="text-richblack-300 text-sm mt-1 mb-3 font-medium line-clamp-2">
                    {instructor?.additionalDetails?.about ||
                      t("pages.course_details.instructor_about_fallback")}
                  </p>
                  <div className="text-sm text-richblack-400 group-hover:underline">
                    {t("pages.course_details.view_profile")}
                  </div>
                </div>
              </Link>
            </div>
            ...
            <div className="pt-4" id="reviews">
              ...
              {isEnrolled && (
                <div className="bg-richblack-800/60 p-6 rounded-2xl border border-richblack-700 mb-8 shadow-lg backdrop-blur-sm">
                  <h3 className="text-lg font-bold text-richblack-5 mb-4">
                    {t("pages.course_details.add_review")}
                  </h3>
                  <div className="flex flex-col gap-4">
                    <div className="flex gap-2 items-center mb-2">
                      <span className="text-richblack-300 text-sm">
                        {t("pages.course_details.rate_course")}
                      </span>
                      ...
                      <span className="text-yellow-50 font-bold ml-2 text-sm">
                        {userRating > 0
                          ? t("pages.course_details.rating_value", {
                              rating: userRating,
                            })
                          : ""}
                      </span>
                    ...
                    <div className="flex justify-end">
                      ...
                      >
                        {isReviewSubmitting
                          ? t("pages.course_details.submitting_review")
                          : t("pages.course_details.submit_review")}
                      </button>
                    </div>
                  </div>
                </div>
                ...
                {ratingAndReviews && ratingAndReviews.length > 0 ? (
                  ratingAndReviews.map((review, index) => {
                    const reviewerName = review?.user
                      ...
                      : t("pages.course_details.anonymous_user");
                    ...
                    return (
                      <div
                        key={index}
                        className="flex gap-4 p-5 border border-richblack-700 bg-richblack-800/30 rounded-2xl hover:bg-richblack-800/50 transition-colors"
                      >
                        <Img
                          src={reviewerImage}
                          alt="User"
                  ...
                  <div className="flex flex-col items-center justify-center py-10 border border-dashed border-richblack-700 rounded-xl bg-richblack-800/20">
                    ...
                    <p className="text-richblack-300">
                      {t("pages.course_details.no_reviews")}
                    </p>
                  </div>
                )}
              </div>
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="text-center">
                    {isEnrolled ? (
                      ...
                      <span className="text-3xl font-bold text-caribbeangreen-200">
                        {t("pages.course_details.free")}
                      </span>
                    ) : (
                  ...
                  {/* Action buttons */}
                  <div
                    className={`grid gap-3 ${isEnrolled || price === 0 ? "grid-cols-1" : "grid-cols-2"}`}
                  >
                    <button
                  ...
                  <div className="space-y-3 pt-4 border-t border-richblack-700">
                    <p className="text-richblack-5 font-semibold text-sm">
                      {t("pages.course_details.course_includes")}
                    </p>
