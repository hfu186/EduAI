...
const { GET_USER_DETAILS_API, GET_USER_ENROLLED_COURSES_API, GET_INSTRUCTOR_DATA_API, GET_ALL_INSTRUCTORS_API, GET_INSTRUCTOR_PROFILE_API } = profileEndpoints
...
export const getInstructorProfile = async (instructorId) => {
  let result = null;
  try {
    const response = await apiConnector("GET", `${GET_INSTRUCTOR_PROFILE_API}/${instructorId}`);
    
    if (!response?.data?.success) {
      throw new Error("Could not fetch instructor profile");
    }
    result = response?.data?.data;
  } catch (error) {
